...
Function IsValPw(Usr As String, Pw As String, Pw2 As String) As Boolean
 IsValPw = False
 If Pw = "" Then
  MsgBox "Enter a password", vbExclamation, MsgT
  Exit Function
 End If
 If Len(Pw) < 6 Then
  MsgBox "The minimum length of passwords is 6 characters", vbExclamation, MsgT
  Exit Function
 End If
 If LCase$(Pw) = LCase$(Usr) Then
  MsgBox "The username cannot be set as password", vbExclamation, MsgT
  Exit Function
 End If
 If Pw = Pw2 Then
  IsValPw = True
 Else
  MsgBox "Passwords do not match", vbExclamation, MsgT
 End If
End Function
Function ValID(vID As String, vTable As String, vField As String) As Boolean
 ValID = False
 If vID = "" Then
  MsgBox "Enter the objects name", vbExclamation, MsgT
  Exit Function
 End If
 If LCase$(vID) = "intendant" Or LCase$(vID) = "system" Then
  MsgBox "This name is reserved, use another one", vbExclamation, MsgT
  Exit Function
 End If
 ...
 Else
  rscon.Close
  MsgBox "An object with this name already exists", vbExclamation, MsgT
 End If
End Function
Private Sub Main()
 If App.PrevInstance = False Then
  If Dir$(App.Path & "\Data\verwalt.cfg") = "" Then
   MsgBox "Config File not found" & vbCrLf & "Cannot connect to database" & vbCrLf & "Enter connection data", vbExclamation, MsgT
   FrmConnDB.Show
   Exit Sub
  End If
  If OpenDB = True Then
