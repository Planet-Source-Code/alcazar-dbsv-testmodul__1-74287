...
Begin VB.Form FrmTestS 
   ...
   Begin VB.Menu mnutest 
      ...
      Begin VB.Menu mnustart 
         Caption         =   "Test starten"
         Enabled         =   0   'False
      End
      Begin VB.Menu mnuauswert 
         Caption         =   "Ergebnisse anzeigen"
         Enabled         =   0   'False
      End
   End
End
...
Private Sub CmdStart_Click()
 ...
 If Val(rstmp) > 1 Then
  If rscon.Fields("anzahl").Value >= Val(rstmp) Then
   ShowMsg "Deine max. Anzahl an Teilnahmen für diesen Test wurde erreicht"
   Exit Sub
  End If
  If Val(tmpval) > 0 And Val(ConvertToTimeStamp(Now())) < (ConvertToTimeStamp(rscon.Fields("trdatum").Value) + Val(tmpval) * 60) Then
   ShowMsg "Für diesen Test wurde eine Wartezeit von " & tmpval & " Minuten" & vbCrLf & "zwischen den Teilnahmen erstellt, diese ist noch nicht abgelaufen"
   Exit Sub
  End If
  rscon.Close
 ...
 Else
  If rscon.Fields("anzahl").Value >= 1 Then
   ShowMsg "Dieser Test kann nur 1x pro Benutzer durchgeführt werden" & vbCrLf & "Du hast bereits teilgenommen"
   Exit Sub
  End If
  rscon.Close
  With TreeView1.SelectedItem
...
Private Function ShowMsg(MsgTxt As String)
 ...
 rstmp = ""
 tmpval = ""
 MsgBox MsgTxt & vbCrLf & "Test wird nicht gestartet", vbExclamation, MsgT
 SetBt False
End Function
