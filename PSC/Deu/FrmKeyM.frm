...
Begin VB.Form FrmKeyM 
   ...
   Begin VB.Label Label3 
      ...
      Caption         =   "Geben Sie Benutzer und Passwort zur Anmeldung an"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
  ...
  Select Case imgLogin.Tag
   Case 0
    If .Role = 2 Then
     ...
     rscon.Close
     Log_Entry "A", "Anmeldung Schuelermodul/Tests: " & .Name, .ID
     Unload Me
     FrmMainS.Show
    Else
     ...
     rscon.Close
     Log_Entry "A", "Anmeldung DBSV: " & .Name, .ID
     Unload Me
     FrmMain.Show
    End If
   Case 1
    If .Role <> 1 Then
     ShowErr 0
    Else
     .RName = "Administration"
     Log_Entry "A", "Anmeldung Admininistration: " & .Name, .ID
     Unload Me
     FrmMainA.Show
    End If
  End Select
...
Private Sub ShowErr(ErrMsg As Integer)
 Log_Entry "A", "Anmeldung DBSV/Verwaltung (Fehler): " & TxtName.Text, 0
 Unload Me
 FrmMsg.LoadFrm 1
End Sub
