...
Begin VB.Form FrmLogFile 
   ...
   Caption         =   "Log Datei - aufgezeichnete Ereignisse"
   ClientHeight    =   7620
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12615
   ...
   Begin VB.Menu mnu 
      ...
      Begin VB.Menu mnusavelog 
         Caption         =   "Logdatei speichern"
      End
      Begin VB.Menu mnudellog 
         Caption         =   "Logdatei leeren"
      End
   End
End
...
Private Sub mnusavelog_Click()
 ...
 SaveLog
 MsgBox "LogDatei gespeichert", vbInformation, MsgT
End Sub
Private Sub mnudellog_Click()
 If vbYes = MsgBox("LogDatei wirklich leeren?", vbExclamation + vbYesNo, MsgT) Then
  If rscon.State = adStateOpen Then rscon.Close
  If vbYes = MsgBox("LogDatei vor dem Leeren speichern?", vbExclamation + vbYesNo, MsgT) Then SaveLog
  dbcon.Execute "DELETE FROM dbsv_adm_logfile;"
  Log_Entry "S", "Ereignislog gelöscht", UsrInf.ID
  Unload Me
 End If
End Sub
...
Private Sub SaveLog()
 If Dir$(App.Path & "\Data\events.log") <> "" Then
  If vbYes = MsgBox("Sicherungsdatei existiert bereits" & vbCrLf & "Überschreiben?", vbExclamation + vbYesNo, MsgT) Then
 ...
 Open App.Path & "\Data\events.log" For Output Access Write As fnr
  Write #fnr, "Typ, Aktion, Datum, UID"
  Do While rscon.EOF = False
